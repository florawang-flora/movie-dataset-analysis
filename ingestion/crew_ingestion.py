...
class CrewIngestion: 
    ...
    def _transformed_table_type(self): 
        ...
        self.df['crew_id'] = pd.to_numeric(
            self.df['crew_id'],
            errors = 'coerce'
        )
        # records bad records if we have, then will print the logging information
        bad_records_crew_id = self.df[self.df['crew_id'].isna()]
        ...
        self.df = self.df.dropna(subset =['crew_id'])

        #convert crew_id to string and strip spaces

        self.df['crew_id']= self.df['crew_id'].astype('int').astype('str')

        #============
        # crew_id whether has primary key 
        #============
        self._check_primary_null_in_column('crew_id')
        self._check_key_incorrect_format_string("crew_id")
        ...
        #============
        # department  gender         job       crew_name  
        #============
        ...
        #    .str.strip()
        #)
        string_columns = ['department', 'gender', 'job', 'crew_name']
...
# make sure the primary key don't have the null value 
    def _clean_duplicates(self):
        ...
        crew_df_clean = self.df.drop_duplicates()
